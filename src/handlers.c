...
extern void kernel_panic(const char *msg, const char *file, int line, uint32_t code);
...
void isr_handler(registers_t *regs) {
    ...
    /* Trigger a graphical kernel panic instead of silently freezing */
    kernel_panic("CPU exception occurred", "isr_handler", 0, regs->int_no);
    /* Does not return */
}
 ...
 *
 * If no HDD is present, we fall back to reading from the CD-ROM drive
 * via ATAPI. This is critical for ISO-only boots (v86, live CD, etc).
*/

extern int ata_read_cdrom_512(uint32_t lba_512, uint32_t count, void *buffer);
...
int disk_read_lba(uint32_t lba, uint32_t count, void *buffer) {
    ...
    /* HDD failed - try CD-ROM via ATAPI */
    if (ata_cdrom_available()) {
        return ata_read_cdrom_512(lba, count, buffer);
    }

    return -1;
}

/* ISO 9660 and El-Torito extra files live on the CD only. Never satisfy these
 * reads from the hard disk — ata_read_sectors succeeds for unrelated HDD LBAs. */
int disk_read_lba_cdrom(uint32_t lba, uint32_t count, void *buffer) {
    if (ata_cdrom_available())
        return ata_read_cdrom_512(lba, count, buffer);
    return -1;
}

/* Raw regions (e.g. wallpaper at fixed HDD LBA). No ATAPI fallback. */
int disk_read_lba_hdd(uint32_t lba, uint32_t count, void *buffer) {
    uint8_t *buf = (uint8_t *)buffer;
    for (uint32_t i = 0; i < count; i++) {
        if (ata_read_sectors(lba + i, 1, buf + i * 512) != 0)
            return -1;
