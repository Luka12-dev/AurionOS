...
static int load_bmp_icon(const char *path, unsigned int *dst, int index) {
    ...
    if (bytes <= 0) {
        ...
        if (!ata_cdrom_available()) return -5; /* CD drive not even found */
        return -1; /* File not found */
    }
    if (bytes < 54) return -2; /* Too small */
    
    /* Verify BMP signature */
    if (bmp_loader_data[0] != 'B' || bmp_loader_data[1] != 'M') return -3; /* Bad signature */
    ...
    if ((bpp != 24 && bpp != 32) || width <= 0 || height <= 0) return -4; /* Bad format */
...
static void try_load_icon(const char *path, unsigned int *dst, uint32_t def_color, int index) {
    ...
    if (res == -5) create_fallback_icon(dst, 0xFFAA00AA); /* Light Purple: Driver/ATA issue */
    else if (res == -1) create_fallback_icon(dst, 0xFFFF00FF); /* Magenta: Path resolution issue */
    else if (res == -2) create_fallback_icon(dst, 0xFFFFFF00); /* Yellow: Too small */
    else if (res == -3) create_fallback_icon(dst, 0xFF00FFFF); /* Cyan: Bad signature */
    else if (res == -4) create_fallback_icon(dst, 0xFFFF8800); /* Orange: Format issue */
    else create_fallback_icon(dst, def_color); /* Default color */
}
