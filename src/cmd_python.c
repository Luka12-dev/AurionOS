 ...
 *   - python             : interactive REPL mode (DOSMODE only)
 *
 * Language features:
 *   - print("...") and print(var)
...
static void py_exec_block(int start, int end_line) {
    int i = start;
    while (i < end_line) {
        /* Check for keyboard interrupt periodically to prevent system freeze */
...
static int py_load_file(const char *filename) {
    ...
    /* Use the OS standard loader which handles current_dir and case-insensitivity */
    int sz = load_file_content(filename, buffer, 8192);
    if (sz <= 0) {
        return -1;
    }
    ...
    py_line_count = 0;
    int pos = 0;
    while (pos < sz && py_line_count < MAX_LINES) {
        int li = 0;
        while (pos < sz && buffer[pos] != '\n' && li < MAX_LINE_LEN - 1) {
            if (buffer[pos] != '\r') py_lines[py_line_count][li++] = buffer[pos];
            pos++;
        }
...
static void py_repl(void) {
    ...
    /* Check if we're in GUI mode - REPL doesn't work in GUI */
    extern int boot_mode_flag;
    if (boot_mode_flag == 0) {
        set_attr(0x0C);
        c_puts("ERROR: Python REPL is not available in GUI mode.\n");
