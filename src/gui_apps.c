...
static void filebrow_on_draw(Window *win) {
    ...
    wm_fill_rect(win, 0, 0, cw, 22, 0xFF000000);
    wm_draw_string(win, 6, 7, state->path, 0xFFFFFFFF, 0xFF000000);

    /* Column headers */
    wm_fill_rect(win, 0, 22, cw, 16, 0xFFF0F0F0);
    wm_draw_string(win, 6, 25, "Name", 0xFF000000, 0xFFF0F0F0);
    wm_draw_string(win, cw - 80, 25, "Type", 0xFF000000, 0xFFF0F0F0);
    wm_draw_string(win, cw - 40, 25, "Size", 0xFF000000, 0xFFF0F0F0);
    /* Separator */
    ...
    for (int i = state->scroll; i < state->entry_count && visible > 0; i++, visible--) {
        ...
        uint32_t bg = sel ? 0xFF22C55E : ((i % 2 == 0) ? 0xFFFFFFFF : 0xFFF5F5FF);
...
static void filebrow_on_key(Window *win, uint16_t key) {
    ...
    int row_h = 18;
    int visible = (ch_h - 58) / row_h; /* subtract header + status bar */
...
static void sysinfo_on_draw(Window *win) {
    ...
    int y = 10;
    wm_draw_string(win, 10, y, "Aurion OS v1.0 Release", 0xFF4ADE80, 0xFF000000);
    y += 20;
...
static void settings_on_draw(Window *win) {
    ...
    if (state->tab == 0) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            wm_fill_rect(win, content_x, y, 120, 24, sel ? WM_COLOR_ACCENT : 0xFF333333);
