...
static inline uint8_t bcd2bin_rtc(uint8_t val) {
    return (uint8_t)(((val >> 4) * 10) + (val & 0x0F));
}

/* Read RTC hour/minute/second using CMOS register B format bits */
static void rtc_read_hms(uint8_t *hours, uint8_t *minutes, uint8_t *seconds) {
    uint8_t regB   = get_cmos_reg(0x0B);
    uint8_t rawH   = get_cmos_reg(0x04);
    uint8_t rawM   = get_cmos_reg(0x02);
    uint8_t rawS   = get_cmos_reg(0x00);
    bool    bcd    = (regB & 0x04) == 0;   /* DM: 0 = BCD */
    bool    hour24 = (regB & 0x02) != 0;   /* 1 = 24-hour */

    uint8_t s = rawS;
    uint8_t m = rawM;
    uint8_t h;
    if (bcd) {
        s = bcd2bin_rtc(rawS & 0x7F);
        m = bcd2bin_rtc(rawM & 0x7F);
    }

    if (hour24) {
        h = bcd ? bcd2bin_rtc(rawH & 0x3F) : (uint8_t)(rawH & 0x3F);
    } else {
        bool pm = (rawH & 0x80) != 0;
        h = bcd ? bcd2bin_rtc(rawH & 0x7F) : (uint8_t)(rawH & 0x7F);
        if (h == 12)
            h = pm ? 12 : 0;
        else if (pm)
            h = (uint8_t)(h + 12);
...
/* BCD to Binary helper (date registers) */
