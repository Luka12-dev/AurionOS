...
#define FS_CONTENT_START_LBA 1200 
...
FileContent file_contents[128] = {0};
...
int load_file_content(const char *filename, char *buffer, int max_len)
{
  ...
  if (ci == -1)
  {
    // puts("[LOAD] Content not found! file_content_count=");
    char l_buf_3[16];
    int_to_str(file_content_count, l_buf_3);
    // puts(l_buf_3);
    // puts("\n");
    return 0; // Empty file
  }
    ...
    if (disk_read_lba(FS_DATA_START_LBA + i, 1, sector) == 0)
    {
      ...
      if (sector[0] != 0)
      {
        ...
        for (int j = 0; fs_table[fs_count].name[j]; j++) {
            if (fs_table[fs_count].name[j] == '\\') fs_table[fs_count].name[j] = '/';
        }
...
static void fs_ensure_documents_dir(void)
{
  ...
  void create_web_file(const char *name, const char *content) {
    ...
    if (content_idx == -1) {
        if (file_content_count >= 64) return;
        content_idx = file_content_count++;
    ...
    const char *content = "";
        ...
        if (f == 0) { /* index.html */
            content = "<html><title>Aurion OS Home</title><body style='background:#000; color:#fff; text-align:center; padding-top:0;'>"
                      ...
                      "</p>"
                      "<div style='margin-top:200; color:#222; font-size:14; text-transform:uppercase; letter-spacing:4; text-align:center;'>Pure Speed. Zero Compromise. | 1.0 Release</div>"
                      "</div>"
                      "</body></html>";
        } else if (f == 1) { /* about.html */
            content = "<html><title>About Aurion OS</title><body style='background:#0a0a0a; color:#f0f0f0; padding:80; text-align:center;'>"
                      ...
                      "<h3 style='color:#4ade80;'>1.0 RELEASE NOTES</h3>"
...
static void fs_init_commands(void)
{
  ...
  bool kb_found = false;
  for (int i = 0; i < fs_count; i++) {
    if (str_cmp(fs_table[i].name, "/Keyboard") == 0) { kb_found = true; break; }
    if (str_cmp(fs_table[i].name, "/Keyboard/") == 0) { kb_found = true; break; }
  }
  ...
  const char *extra_dirs[] = { "/Music", "/Pictures", "/Downloads", "/Videos", "/Code", "/System", "/Keyboard" };
  for (int d = 0; d < 7; d++) {
    bool found = false;
    for (int i = 0; i < fs_count; i++) {
      if (str_cmp(fs_table[i].name, extra_dirs[d]) == 0) { found = true; break; }
    }
  ...
  if (ulen > 0)
  {
    ...
    for (int i = 0; i < fs_count; i++) {
        if (str_cmp(fs_table[i].name, hpath) == 0) { home_exists = true; break; }
    }
...
{
  (void)args;
  puts("AurionOS Version 1.0 Release\n");
...
static int cmd_cd(const char *args)
{
  ...
  for (int i = 0; i < fs_count; i++)
  {
    if ((str_cmp(fs_table[i].name, full_path) == 0 || str_cmp(fs_table[i].name, alt_path) == 0) 
        && fs_table[i].type == 1)
    {
      found_idx = i;
      break;
...
static int cmd_mem(const char *args)
{
  ...
  puts("  Total RAM:    ");
  int_to_str(total_ram, v_buf); puts(v_buf);
  puts(" bytes (");
  int_to_str(total_kb / 1024, v_buf); puts(v_buf);
  puts(" MB)\n");
...
static int cmd_tree(const char *args)
{
  ...
  for (int i = 0; i < fs_count; i++)
  {
    ...
    if (*rel_path == 0) puts("."); /* current dir itself */
    else puts(rel_path);
...
static int cmd_label(const char *args)
{
  ...
  if (new_label[0] == 0)
  {
    ...
    while (pos < 11) {
        ...
        char c = (char)(k & 0xFF);
        if (c == 13 || c == 10) break;
        if (c == 8 && pos > 0) { pos--; puts("\b \b"); }
        if (c >= 32 && c <= 126) { input[pos++] = c; putc(c); }
    }
    input[pos] = 0;
    putc('\n');
    if (pos > 0) {
...
static int cmd_chkdsk(const char *args)
{
  ...
  for (int i = 0; i < fs_count; i++) {
      ...
      if (fs_table[i].parent_idx != 0xFFFF && fs_table[i].parent_idx >= fs_count) {
          errors++;
          if (fix) fs_table[i].parent_idx = 0xFFFF; // Fix: Reset to root
      }
  }
  ...
  char v_buf[32];
  puts("\nDisk Scan Results:\n");
  puts("  Total Files:      "); int_to_str(fs_count, v_buf); puts(v_buf); puts("\n");
  
  uint32_t total_size = 0;
  for (int i = 0; i < fs_count; i++) total_size += fs_table[i].size;
  
  puts("  Total Size:       "); int_to_str(total_size, v_buf); puts(v_buf); puts(" bytes\n");
  puts("  Content Slots:    "); int_to_str(file_content_count, v_buf); puts(v_buf); puts("/64\n");
  ...
  if (errors == 0) {
      ...
      set_attr(0x0C);
      puts("\nFound "); int_to_str(errors, v_buf); puts(v_buf); puts(" inconsistencies!\n");
  ...
  puts("       "); puts(v_buf); puts(" KB in files.\n");
  int_to_str((102400 - (total_size / 1024)), v_buf);
  puts("       "); puts(v_buf); puts(" KB available on disk.\n");
...
static int cmd_diskpart(const char *args)
{
  ...
  if (has_mbr) {
      ...
      for (int i = 0; i < 4; i++) {
          if (entries[i].sector_count > 0) {
              found++;
              puts("Partition "); int_to_str(i + 1, v_buf); puts(v_buf);
              puts("    ");
              
              /* Type string */
              if (entries[i].type == 0x83) puts("Linux/AurionFS  ");
              else if (entries[i].type == 0x07) puts("NTFS/HPFS       ");
              else if (entries[i].type == 0x0B || entries[i].type == 0x0C) puts("FAT32           ");
              else if (entries[i].type == 0xEE) puts("GPT Protective  ");
              else {
                  puts("Unknown (0x");
                  /* Simple hex for type */
                  char hex[] = "0123456789ABCDEF";
      ...
      if (found == 0) puts("No active partitions found in MBR.\n");
      puts("\n");
  } else {
  ...
  puts("  [1000-1127] File Registry Table ("); int_to_str(fs_count, v_buf); puts(v_buf); puts("/");
  int_to_str(FS_MAX_FILES, v_buf); puts(v_buf); puts(" slots)\n");
  puts("  [628-659] User Security Table ("); int_to_str(user_count, v_buf); puts(v_buf); puts("/");
  int_to_str(FS_MAX_USERS, v_buf); puts(v_buf); puts(" users)\n");
...
static int cmd_ps(const char *args)
{
  ...
  int_to_str(shell_kb, v_buf); puts(v_buf); puts(" KB       ");
  puts(current_user); puts("\n");
  ...
  for (int i = 0; i < count; i++) {
      ...
      puts(name);
      len = str_len(name);
      for (int k=0; k<17-len; k++) putc(' ');

      /* State */
      if (win->focused) puts("FOCUSED      ");
      else if (win->visible) puts("VISIBLE      ");
      else puts("MINIMIZED    ");
...
static int cmd_kill(const char *args)
{
  ...
  if (pid >= 100) {
      ...
      for (int i = 0; i < count; i++) {
          ...
          if (win && win->app_id == app_id) {
              puts("Stopping process: "); puts(win->title); puts("...\n");
  ...
  puts("Error: No process found with PID "); puts(pid_str); puts("\n");
  return -1;
}
...
static int cmd_uname(const char *a)
{
  ...
  {
    puts("AurionOS AurionOS 1.0.0 i386 AurionOS/x86\n");
  }
...
static int cmd_sort(const char *a)
{
  char name[64];
  a = get_token(a, name, 64);
  if (name[0] == 0) { puts("Usage: SORT <file>\n"); return -1; }
  char full[256];
  build_full_path(name, full);
  int fi = -1;
  for (int i = 0; i < fs_count; i++) {
    if (str_cmp(fs_table[i].name, full) == 0 && fs_table[i].type == 0) { fi = i; break; }
  }
  if (fi < 0) { puts("File not found\n"); return -1; }
  int ci = -1;
  for (int i = 0; i < file_content_count; i++) {
    if ((int)file_contents[i].file_idx == fi) { ci = i; break; }
  }
  if (ci < 0) return 0;
...
static int cmd_uniq(const char *a)
{
  char name[64];
  a = get_token(a, name, 64);
  if (name[0] == 0) { puts("Usage: UNIQ <file>\n"); return -1; }
  char full[256];
  build_full_path(name, full);
  int fi = -1;
  for (int i = 0; i < fs_count; i++) {
...
static int cmd_sudo(const char *args)
{
  ...
  while (c_kb_hit()) c_getkey(); /* Flush any leftover keys */
...
static int cmd_wget(const char *args)
{
  ...
  while (*s && (d - req) < 500)
    *d++ = *s++;
  s = "\r\nUser-Agent: AurionOS/1.0 Release\r\nConnection: close\r\nAccept: */*\r\n\r\n";
  while (*s && (d - req) < 511)
    *d++ = *s++;
  *d = 0;
