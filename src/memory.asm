...
mem_init:
    ...
    ; Debug: Log heap parameters to serial
    push dword .debug_msg
    call serial_puts
    add esp, 4
    ...
    jmp .skip_msg
.debug_msg db "[MEM] Initializing heap...", 10, 0
.skip_msg:
    ...
    ; Also update the first MCB to be part of the free list
    ; prev = NULL, next = NULL (already cleared)
    ; size = payload size (already set)
    ; magic = MCB_MAGIC (already set)
    ; flags = MCB_FLAG_FREE (already set)
...
kmalloc:
    push ebp
    mov ebp, esp
    sub esp, 4              ; Local for aligned size: [ebp - 4]
    push ebx
    push ecx
    push edx
    push esi
    ...
    add esp, 8
    jmp .start_km
.km_msg db "[MEM] kmalloc request size: ", 0
.start_km:
    mov eax, [ebp + 8]      ; RELOAD requested size from stack
    test eax, eax
    jz .alloc_failed
    ...
    add eax, 3
    and eax, 0xFFFFFFFC
    mov [ebp - 4], eax      ; Save aligned size (at [ebp-4])

    ; Search all blocks from base (physical chain)
    mov esi, [heap_base]
...
.allocate_block:
    ...
    mov eax, [ebp - 4]      ; RELOAD aligned size from [ebp-4]
    mov ebx, [esi + 8]      ; block size
...
.use_whole_block:
    ; No removal needed - we maintain the physical chain
.remove_done:
    ; Mark block as used
    mov dword [esi + 16], MCB_FLAG_USED
...
.alloc_success:
    ...
    mov ebx, [ebp - 4]     ; requested size (aligned)
...
.add_to_free_list:
    ; Head insertion removed to maintain physical chain stability
    jmp .done
...
.debug_msg       db "mem_init: heap_base=0x", 0  ; Need to print hex manually in ASM or just string
