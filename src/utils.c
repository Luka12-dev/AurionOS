...
/* Forward declarations (needed for fortify stubs). */
...
/* Toolchain/glibc fortify stubs (freestanding kernel, no libc). */
...
uint16_t c_getkey(void) {
    if (terminal_last_char != 0) {
        uint16_t ret = (uint16_t)terminal_last_char;
...
uint16_t c_getkey_nonblock(void) {
    if (c_kb_hit()) {
        return c_getkey();
    }
    return 0;
