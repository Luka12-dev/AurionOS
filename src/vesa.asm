...
.step2:
    ...
    ; Set BPP (force 32 for stable QEMU/Bochs LFB writes)
    mov dx, VBE_DISPI_IOPORT_INDEX
    ...
    ; Program virtual dimensions to match visible dimensions.
    ; QEMU/Bochs can otherwise keep a stale virtual width (pitch),
    ; which manifests as horizontal corruption bands near the bottom.
    mov dx, VBE_DISPI_IOPORT_INDEX
    ...
    ; Calculate pitch: virt_width * 4 (32-bpp)
    movzx ebx, ax
    mov eax, 4
    imul eax, ebx
...
vesa_set_mode:
    ...
    movzx eax, word [ebp+8]    ; width from stack
    movzx ebx, word [ebp+12]   ; height from stack
    movzx edx, byte [ebp+16]   ; bpp from stack
