// Developer Note: Still Working on 'Make' and 'Python', AurionOS v1 Beta don't have full integrated logic.
...
extern MakeFSEntry fs_table[];

/* File content access */
#define MAKE_MAX_FILE_SIZE 4096
typedef struct {
    uint16_t file_idx;
    uint32_t size;
    char data[MAKE_MAX_FILE_SIZE];
} MakeFileContent;
extern MakeFileContent file_contents[];
...
int cmd_make(const char *args) {
    ...
    /* Also search for any Makefile variant in the current directory,
       case-insensitively. Covers "makefile", "MAKEFILE", "GNUmakefile", etc.
       Compare the last component of each fs_table entry against known names. */
    if (found_idx < 0) {
        ...
        for (int i = 0; i < fs_count && found_idx < 0; i++) {
            ...
            /* Must be a direct child (no more backslashes) */
            bool sub = false;
