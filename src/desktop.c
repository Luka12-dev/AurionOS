...
extern void app_task_manager_create(void);
extern void app_audio_manager_create(void);
...
void scan_desktop_items(void)
{
    ...
    {
        if (!desktop_str_startswith(fs_table[i].name, desktop_prefix))
            continue;
...
void load_app_hidden_state(void)
{
    ...
    int plen = 0;
    while (hidden_prefix[plen])
        plen++;

    for (int i = 0; i < fs_count; i++)
    {
        if (!desktop_str_startswith(fs_table[i].name, hidden_prefix))
            continue;
...
{
    if (desktop_toast_ttl <= 0 || desktop_toast[0] == 0)
        return;
    int len = 0;
    while (desktop_toast[len])
        len++;
    int tw = len * 8 + 24;
    if (tw > screen_w - 20)
        tw = screen_w - 20;
...
static bool desktop_nf_handle_key(uint16_t key)
{
    ...
    if (lo == 13)
    {
        ...
        int pi = 0;
        while (prefix[pi] && pi < 95)
        {
            full_path[pi] = prefix[pi];
            pi++;
        }
        for (int u = 0; u < nlen && pi < 95; u++)
            full_path[pi++] = name[u];
        full_path[pi] = 0;
                       ...
                       int screen_w, int screen_h)
{
    ...
    if (right_click && !ctx_open)
    {
        ...
        for (int i = 0; i < wm_get_window_count(); i++)
        {
            ...
            if (!w || !w->visible || w->state == WM_STATE_MINIMIZED)
                continue;
        ...
        ctx_x = mx;
        ctx_y = my;
        if (ctx_x + pw > screen_w)
            ctx_x = screen_w - pw - 4;
        if (ctx_y + ph > screen_h)
            ctx_y = screen_h - ph - 4;
        if (ctx_x < 4)
            ctx_x = 4;
        if (ctx_y < WM_MENUBAR_HEIGHT + 4)
            ctx_y = WM_MENUBAR_HEIGHT + 4;
    ...
    if (click)
    {
        ...
        else if (ctx_hover == 2 || ctx_hover == 3)
            app_settings_create();
        else if (ctx_hover == 4)
            desktop_nf_begin();
        ctx_open = false;
