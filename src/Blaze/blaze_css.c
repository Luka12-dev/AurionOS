...
/* Author-rule fields that use sentinel "unset" must be initialized for every rule */
static void css_rule_init_author_unset(CSSRule *rule)
{
    rule->text_align = -1;
    rule->width_px = 0;
    rule->height_px = 0;
    rule->border_radius = 0;
    rule->rule_font_bold = false;
    rule->rule_font_italic = false;
    rule->rule_font_weight_set = false;
    rule->rule_font_style_set = false;
    rule->display_specified = false;
    rule->display = DISPLAY_UNSET;
}
...
void blaze_apply_default_styles(BlazeTab *tab)
{
    ...
    css_rule_init_author_unset(rule);
...
{
    if (rule->bg_color != 0)
        node->bg_color = rule->bg_color;
    if (rule->fg_color != 0)
        node->fg_color = rule->fg_color;
    if (rule->border_width > 0)
    {
        node->border_width = rule->border_width;
        node->border_color = rule->border_color;
    }
    if (rule->border_radius > 0)
        node->border_radius = rule->border_radius;
    if (rule->text_align >= 0)
        node->text_align = rule->text_align;
    if (rule->width_px > 0)
        node->fixed_w = rule->width_px;
    if (rule->height_px > 0)
        node->fixed_h = rule->height_px;
    if (rule->font_size > 0)
        node->font_size = rule->font_size;
    if (rule->rule_font_weight_set)
        node->is_bold = rule->rule_font_bold;
    if (rule->rule_font_style_set)
        node->is_italic = rule->rule_font_italic;
    if (rule->display_specified && rule->display != DISPLAY_UNSET)
        node->display = rule->display;
    for (int i = 0; i < 4; i++)
    {
        if (rule->padding[i] > 0)
            node->padding[i] = rule->padding[i];
        if (rule->margin[i] > 0)
            node->margin[i] = rule->margin[i];
    }
    /* Grid properties */
    if (rule->grid_gap > 0)
        node->grid_gap = rule->grid_gap;
    if (rule->grid_column_count > 0)
    {
        node->grid_column_count = rule->grid_column_count;
        for (int i = 0; i < rule->grid_column_count && i < 16; i++)
            node->grid_template_columns[i] = rule->grid_template_columns[i];
    }
    if (rule->grid_row_count > 0)
    {
        node->grid_row_count = rule->grid_row_count;
        for (int i = 0; i < rule->grid_row_count && i < 16; i++)
            node->grid_template_rows[i] = rule->grid_template_rows[i];
    }
    if (rule->grid_column_start > 0)
        node->grid_column_start = rule->grid_column_start;
    if (rule->grid_column_end > 0)
        node->grid_column_end = rule->grid_column_end;
    if (rule->grid_row_start > 0)
        node->grid_row_start = rule->grid_row_start;
    if (rule->grid_row_end > 0)
        node->grid_row_end = rule->grid_row_end;
    if (rule->is_grid_container)
...
static void apply_property(CSSRule *rule, const char *prop, const char *val)
{
    ...
    else if (blaze_str_cmp(prop, "border") == 0)
    {
        ...
        if (*p >= '0' && *p <= '9')
        {
            ...
            /* Parse color */
            if (*p)
                rule->border_color = blaze_parse_color(p);
        }
    }
    /* Font size */
    ...
    {
        rule->display_specified = true;
        if (blaze_str_cmp(val, "none") == 0)
        {
            rule->display = DISPLAY_NONE;
            rule->display_block = false;
        }
        else if (blaze_str_cmp(val, "inline") == 0 || blaze_str_cmp(val, "inline-block") == 0)
        {
            rule->display = DISPLAY_INLINE;
            rule->display_block = false;
        }
        else
        {
            rule->display = DISPLAY_BLOCK;
            rule->display_block = true;
        }
    }
    else if (blaze_str_cmp(prop, "text-align") == 0)
    {
        if (blaze_str_cmp(val, "center") == 0)
            rule->text_align = 1;
        else if (blaze_str_cmp(val, "right") == 0)
            rule->text_align = 2;
        else if (blaze_str_cmp(val, "justify") == 0)
            rule->text_align = 3;
        else
            rule->text_align = 0;
    }
    else if (blaze_str_cmp(prop, "width") == 0)
    {
        const char *q = val;
        rule->width_px = parse_int(&q);
    }
    else if (blaze_str_cmp(prop, "height") == 0)
    {
        const char *q = val;
        rule->height_px = parse_int(&q);
    }
    else if (blaze_str_cmp(prop, "border-radius") == 0)
    {
        const char *q = val;
        rule->border_radius = parse_int(&q);
    }
    else if (blaze_str_cmp(prop, "font-weight") == 0)
    {
        rule->rule_font_weight_set = true;
        if (blaze_str_cmp(val, "bold") == 0 || blaze_str_cmp(val, "bolder") == 0)
            rule->rule_font_bold = true;
        else if (blaze_str_cmp(val, "normal") == 0 || blaze_str_cmp(val, "lighter") == 0)
            rule->rule_font_bold = false;
        else
        {
            const char *q = val;
            int w = parse_int(&q);
            rule->rule_font_bold = (w >= 600);
        }
    }
    else if (blaze_str_cmp(prop, "font-style") == 0)
    {
        rule->rule_font_style_set = true;
        rule->rule_font_italic =
            (blaze_str_cmp(val, "italic") == 0 || blaze_str_cmp(val, "oblique") == 0);
    }
    ...
    else if (blaze_str_cmp(prop, "display") == 0)
    {
        ...
        else if (blaze_str_cmp(val, "grid") == 0)
        {
            rule->display = DISPLAY_GRID;
            rule->display_block = true;
            rule->is_grid_container = true;
        }
        else if (blaze_str_cmp(val, "inline-grid") == 0)
        {
            rule->display = DISPLAY_INLINE_GRID;
            rule->display_block = true;
            rule->is_grid_container = true;
        }
        else
    ...
    else if (blaze_str_cmp(prop, "grid-template-columns") == 0 || blaze_str_cmp(prop, "grid-template-rows") == 0)
    {
        const char *p = val;
        int *target = (blaze_str_cmp(prop, "grid-template-columns") == 0) ? rule->grid_template_columns : rule->grid_template_rows;
        int count = 0;
        while (*p && count < 16)
        {
            p = skip_whitespace(p);
            ...
            {
                target[count++] = parse_int(&p);
                while (*p && *p != ' ' && *p != '\t')
                    p++;
            }
            else if (*p == 'a' && *(p + 1) == 'u' && *(p + 2) == 't' && *(p + 3) == 'o')
            {
                target[count++] = -2;
                p += 4;
            }
        ...
        if (blaze_str_cmp(prop, "grid-template-columns") == 0)
            rule->grid_column_count = count;
        else
            rule->grid_row_count = count;
    }
    else if (blaze_str_cmp(prop, "gap") == 0 || blaze_str_cmp(prop, "grid-gap") == 0)
    {
        const char *p = val;
        rule->grid_gap = parse_int(&p);
    }
    else if (blaze_str_cmp(prop, "grid-column-start") == 0)
    {
        const char *p = val;
        rule->grid_column_start = parse_int(&p);
    }
    else if (blaze_str_cmp(prop, "grid-column-end") == 0)
    {
        const char *p = val;
        rule->grid_column_end = parse_int(&p);
    }
    else if (blaze_str_cmp(prop, "grid-row-start") == 0)
    {
        const char *p = val;
        rule->grid_row_start = parse_int(&p);
    }
    else if (blaze_str_cmp(prop, "grid-row-end") == 0)
    {
        const char *p = val;
        rule->grid_row_end = parse_int(&p);
    }
    else if (blaze_str_cmp(prop, "grid-column") == 0)
    {
        const char *p = val;
        rule->grid_column_start = parse_int(&p);
        while (*p && *p != ' ' && *p != '/')
    ...
    else if (blaze_str_cmp(prop, "grid-row") == 0)
    {
        const char *p = val;
        rule->grid_row_start = parse_int(&p);
        while (*p && *p != ' ' && *p != '/')
    ...
    else if (blaze_str_cmp(prop, "justify-items") == 0)
    {
        if (blaze_str_cmp(val, "start") == 0)
            rule->justify_items = 0;
        else if (blaze_str_cmp(val, "end") == 0)
            rule->justify_items = 1;
        else if (blaze_str_cmp(val, "center") == 0)
            rule->justify_items = 2;
        else if (blaze_str_cmp(val, "stretch") == 0)
            rule->justify_items = 3;
    }
    else if (blaze_str_cmp(prop, "align-items") == 0)
    {
        if (blaze_str_cmp(val, "start") == 0)
            rule->align_items = 0;
        else if (blaze_str_cmp(val, "end") == 0)
...
{
    if (!css || len == 0 || tab->rule_count >= BLAZE_MAX_CSS_RULES)
        return;
