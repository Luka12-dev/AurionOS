...
void blaze_navigate(BlazeState *state, const char *url) {
    ...
    blaze_str_copy(tab->url, url, BLAZE_MAX_URL_LEN);
    tab->loading = true;
    tab->scroll_y = 0;
    ...
    if (state->history_count < BLAZE_MAX_HISTORY) {
        blaze_str_copy(state->history[state->history_count], url, BLAZE_MAX_URL_LEN);
        state->history_count++;
    ...
    if (blaze_str_starts_with(url, "file://")) {
        is_local = true;
        file_path = url + 7; /* Skip "file://" */
    } else if (url[0] == '/') {
        is_local = true;
        file_path = url;
    } else if (url[0] && !blaze_str_starts_with(url, "http")) {
        ...
        blaze_str_copy(absolute_path + plen, url, BLAZE_MAX_URL_LEN - plen);
    ...
    if (is_local) {
            ...
            if (bytes_read > 0) {
                ...
                if (blaze_str_cmp(file_path, "/Documents/index.html") == 0) {
                    ...
                    const char *fallback = "<html><title>Aurion OS</title><body style='background:#000; color:#fff; text-align:center; padding-top:100;'>"
                                          ...
                                          "<p style='color:#888; font-size:24;'>Premium Web Experience v1.0</p>"
        ...
        /* Fetch from network */
        result = blaze_fetch(url, &content, &content_len);
    }
    ...
    if (result == 0 && content) {
        ...
        const char *error_html = 
            "<html><body style='background:#000;color:#0f0;padding:40px;font-family:monospace;'>"
            "<h1 style='color:#f00;'>Page Load Error</h1>"
            "<p>Failed to load the requested page.</p>"
            "<p>URL: ";
        
        /* Build error page */
        int err_len = blaze_str_len(error_html) + blaze_str_len(url) + 200;
        char *err_page = (char *)kmalloc(err_len);
        if (err_page) {
            int pos = 0;
            for (int i = 0; error_html[i]; i++) err_page[pos++] = error_html[i];
            for (int i = 0; url[i]; i++) err_page[pos++] = url[i];
            const char *end = "</p><p>Try loading a local file like <strong>/Documents/index.html</strong></p></body></html>";
...
void blaze_handle_key(BlazeState *state, uint16_t key) {
    ...
    if (ascii == 12) { /* Ctrl+L */
        ...
        blaze_str_copy(state->address_bar, tab->url, BLAZE_MAX_URL_LEN);
        state->address_bar_cursor = blaze_str_len(state->address_bar);
        return;
    }
