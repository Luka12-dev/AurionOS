...
io_init:
    ...
    ; Aggressively clear screen FIRST
    ; Fill entire VGA buffer with spaces
    mov edi, [video_base]
    mov ecx, 4000  ; 80x25 x 2 = 4000 bytes (double to be safe)
...
vga_cls:
    ...
    ; Clear the entire VGA text buffer (0xB8000 - 0xB8FA0)
    ; Do it THREE times to ensure all artifacts are gone
    mov edi, [video_base]
    ...
    ; Third pass - absolutely ensure clean
    mov edi, [video_base]
    mov ecx, 2000
    mov ah, [default_attr]
    mov al, ' '
    ...
    ; Also clear the extended buffer area (just in case)
    mov edi, 0xB8FA0
    mov ecx, 96  ; Clear a bit beyond
    mov ax, 0x0720
    rep stosw
