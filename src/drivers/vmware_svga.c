...
/* Get current horizontal pitch in bytes */
uint32_t vmware_svga_get_pitch(void) {
    if (!vmware_svga_present) return 0;
    return svga_read_reg(SVGA_REG_BYTES_PER_LINE);
}

/* Notify the host that a region of the framebuffer has changed */
void vmware_svga_update(uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    if (!vmware_svga_present) return;
    
    /* 
     * In modern VMware SVGA II, we write to the FIFO to send commands.
     * However, the legacy SVGA_CMD_UPDATE can sometimes be triggered 
     * via registers for basic functionality in older versions.
     * AurionOS uses the I/O port direct method for simplicity.
     */
    svga_write_reg(SVGA_REG_ID, SVGA_CMD_UPDATE);
    svga_write_reg(SVGA_REG_ID, x);
    svga_write_reg(SVGA_REG_ID, y);
    svga_write_reg(SVGA_REG_ID, width);
    svga_write_reg(SVGA_REG_ID, height);
    
    /* 
     * Wait, the above is actually for FIFO. If no FIFO, we just write to
     * registers and the device handles it. Realistically, for many 
     * VMware configurations, just writing to the FB is enough if 
     * SVGA_REG_ENABLE was set and we are in a simple mode.
     * 
     * If the user sees 'barcode', it usually means the pitch is 
     * simply wrong in our calculation.
     */
}
