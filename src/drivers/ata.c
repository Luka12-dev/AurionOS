                                ...
                                uint8_t slave, AtaDrive *drv)
{
    ...
    /* Floating bus check — no drive attached */
    ...
    /* Check device signature in LBA_MID / LBA_HI */
    ...
    if (lba_mid == 0x3C && lba_hi == 0xC3) {
        /* SATA device behind bridge — treat as ATAPI for safety */
        drv->present  = true;
        drv->is_atapi = true;
        return true;
    }
...
int ata_init(void) {
    ...
    for (int bus = 0; bus < 2; bus++) {
        ...
        for (int role = 0; role < 2; role++) {
            ...
            /* Log what we found - DISABLED for clean boot */
            /*
