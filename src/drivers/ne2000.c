/*
 * NE2000-compatible Ethernet Driver for AurionOS
 * 
...
static network_interface_t ne2000_iface;
...
int ne2000_send_shim(network_interface_t *iface, const uint8_t *data, uint32_t len);
int ne2000_recv_shim(network_interface_t *iface, uint8_t *data, uint32_t max_len);
...
int ne2000_init(void) {
    ...
    ne2000_iface.name[0] = 'n';
    ne2000_iface.name[1] = 'e';
    ne2000_iface.name[2] = '0';
    ne2000_iface.name[3] = '\0';
    for (int i = 0; i < 6; i++)
        ne2000_iface.mac_addr[i] = ne2000_mac[i];
    ne2000_iface.link_up = true;
    
    /* Use local shim to match function signatures */
    ne2000_iface.send_packet = ne2000_send_shim;
    ne2000_iface.recv_packet = ne2000_recv_shim;
    
    extern int netif_register(network_interface_t * iface);
    netif_register(&ne2000_iface);
    
    c_puts("[NE2K] Initialized\n");
    return 0;
}

/* Shims to match network_interface_t function pointers */
int ne2000_send_shim(network_interface_t *iface, const uint8_t *data, uint32_t len) {
    (void)iface;
    return ne2000_send(data, (uint16_t)len);
}

int ne2000_recv_shim(network_interface_t *iface, uint8_t *data, uint32_t max_len) {
    (void)iface;
    return ne2000_recv(data, (uint16_t)max_len);
}
