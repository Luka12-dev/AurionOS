...
static void calibrate_tsc(void)
{
    ...
    if (tsc_per_us == 0)
        tsc_per_us = 1000; /* Fallback: assume ~1GHz */
}
...
{
    if (mouse.event_head == mouse.event_tail)
        return false;
...
static void mouse_apply_movement(int dx, int dy)
{
    ...
    /* Clamp movement at screen edges to prevent cursor escape:
     * If already at left edge and trying to move left, discard movement
     * If already at right edge and trying to move right, discard movement
     * Same for Y axis. This prevents the host mouse from escaping QEMU. */
    if (mouse.x == 0 && x_pixels < 0)
    {
        x_pixels = 0;
        mouse.x_accum = 0; /* Clear accumulator to prevent push-against-wall */
    }
    if (mouse.x == mouse.limit_w - 1 && x_pixels > 0)
    {
        x_pixels = 0;
        mouse.x_accum = 0;
    }
    if (mouse.y == 0 && y_pixels < 0)
    {
        y_pixels = 0;
        mouse.y_accum = 0;
    }
    if (mouse.y == mouse.limit_h - 1 && y_pixels > 0)
    {
        y_pixels = 0;
        mouse.y_accum = 0;
    }
...
/* Clamp mouse position to screen bounds (belt-and-suspenders) */
static void mouse_clamp_position(void)
{
    ...
    if (mouse.x >= mouse.limit_w)
    {
        mouse.x = mouse.limit_w - 1;
        mouse.x_accum = 0;
    }
    if (mouse.y < 0)
    {
    ...
    if (mouse.y >= mouse.limit_h)
    {
        mouse.y = mouse.limit_h - 1;
        mouse.y_accum = 0;
    }
}
...
static bool ps2_mouse_cmd(uint8_t cmd)
{
    ...
    while (timeout-- > 0)
    {
        ...
        if (b == 0xFA)
            return true; /* ACK */
        if (b == 0xFE)
            return false; /* Resend request = error */
        if (b == 0xFC)
            return false; /* Error */
        if (b == -1)
        {
...
static void ps2_process_packet(void)
{
    ...
    if (status_byte & 0x10)
        dx |= 0xFF00; /* X sign bit */
    if (status_byte & 0x20)
        dy |= 0xFF00; /* Y sign bit */
    ...
    else if (mouse.ps2_type == PS2_MOUSE_TYPE_EXPLORER)
    {
        ...
        if (dz & 0x08)
            dz |= 0xF0; /* Sign extend 4-bit value */
        if (mouse.ps2_packet[3] & 0x10)
            extra_buttons |= MOUSE_BUTTON_4;
        if (mouse.ps2_packet[3] & 0x20)
            extra_buttons |= MOUSE_BUTTON_5;
    }
...
{
    if (uhci_controller_count >= MAX_UHCI_CONTROLLERS)
        return false;
...
{
    if (hc->td_pool_used >= UHCI_TD_POOL_SIZE)
        return NULL;
...
static void uhci_setup_interrupt_polling(uhci_controller_t *hc, usb_mouse_device_t *dev)
{
    ...
    if (uhci_port_is_low_speed(hc, 0))
        td->status |= UHCI_TD_LS;
...
{
    if (!dev->td_pool || !dev->transfer_buffer)
        return;
...
{
    if (ohci_controller_count >= MAX_OHCI_CONTROLLERS)
        return false;
...
{
    if (hc->ed_pool_used >= OHCI_ED_POOL_SIZE)
        return NULL;
...
{
    if (hc->td_pool_used >= OHCI_TD_POOL_SIZE)
        return NULL;
...
static void ohci_setup_interrupt_polling(ohci_controller_t *hc, usb_mouse_device_t *dev)
{
    ...
    if (dev->toggle)
        td->control |= OHCI_TD_TOGGLE_1;
    else
        td->control |= OHCI_TD_TOGGLE_0;
...
static void ohci_poll_mouse(ohci_controller_t *hc, usb_mouse_device_t *dev)
{
    ...
    if (cc == 0)
    {
        ...
        if (actual_len >= 3)
        {
            ...
            mouse_event_t evt = {
                .x = mouse.x, .y = mouse.y, .dx = dx, .dy = dy, .dz = dz, .buttons = mouse.buttons, .absolute = false, .timestamp = rdtsc()};
            mouse_queue_event(&evt);
            mouse.total_packets++;
        }
    }
...
static bool ehci_init_controller(int pci_idx)
{
    ...
    while (timeout-- > 0)
    {
        if (mmio_read32(hc->op_base + EHCI_OP_USBSTS) & EHCI_STS_HALT)
            break;
        mdelay(1);
    }
    ...
    while (timeout-- > 0)
    {
        if (!(mmio_read32(hc->op_base + EHCI_OP_USBCMD) & EHCI_CMD_HCRESET))
            break;
        mdelay(1);
    }
                                 ...
                                 usb_setup_packet_t *setup, void *data, uint16_t data_len)
{
    ...
    for (int i = 1; i < 5; i++)
        setup_qtd->buffer[i] = 0;
    for (int i = 0; i < 5; i++)
        setup_qtd->ext_buffer[i] = 0;
    ...
    for (int i = 0; i < 5; i++)
    {
        status_qtd->buffer[i] = 0;
        status_qtd->ext_buffer[i] = 0;
    }
    ...
    while (timeout_ms-- > 0)
    {
        ...
        if (!(status_qtd->token & EHCI_QTD_STATUS_ACTIVE))
            break;
        if (status_qtd->token & EHCI_QTD_STATUS_HALTED)
            break;
        mdelay(1);
    }
...
static int xhci_wait_event(xhci_controller_t *hc, uint32_t *out_param, uint32_t *out_status, int timeout_ms)
{
    while (timeout_ms-- > 0)
    {
        ...
        if ((evt->control & 0x01) == (uint32_t)(hc->evt_ring_cycle & 0x01))
        {
            /* Event available */
            if (out_param)
                *out_param = evt->param_lo;
            if (out_status)
                *out_status = evt->status;
...
static bool xhci_init_controller(int pci_idx)
{
    ...
    if ((bar0 & 0x01) == 0)
    {
        ...
        if (type == 0x02)
        {
            ...
            hc->base = (volatile uint8_t *)(uintptr_t)bar64;
        }
        else
        {
            hc->base = (volatile uint8_t *)(uintptr_t)(bar0 & 0xFFFFFFF0);
        }
    }
    else
    {
    ...
    while (timeout-- > 0)
    {
        if (mmio_read32(hc->op_base + XHCI_OP_USBSTS) & XHCI_STS_HCH)
            break;
        mdelay(1);
    }
...
static bool xhci_address_device(xhci_controller_t *hc, int slot, int port, int speed)
{
    ...
    case 2:
        max_packet = 8;
        break; /* Low speed: 8 bytes */
    case 1:
        max_packet = 64;
        break; /* Full speed: 8-64 */
    case 3:
        max_packet = 64;
        break; /* High speed: 64 */
    case 4:
        max_packet = 512;
        break; /* Super speed: 512 */
    default:
        max_packet = 64;
        break;
    }
...
{
    if (!dev->transfer_buffer || !hc->int_rings[slot])
        return;
...
{
    if (usb_mouse_count >= MAX_USB_MICE)
        return false;
    ...
    if (ctrl_type == 3)
    {
        ...
        if (!xhci_address_device(hc, slot, port, speed))
            return false;
...
int mouse_init(void)
{
    ...
    /* Sensitivity: 1:1 speed (100% - match host cursor exactly) */
    mouse.sensitivity_num = 1;
    mouse.sensitivity_den = 1;
    ...
    for (int i = 0; i < usb_controller_count; i++)
    {
        ...
        {
        case 0:
            uhci_init_controller(i);
            break;
        case 1:
            ohci_init_controller(i);
            break;
        case 2:
            ehci_init_controller(i);
            break;
        case 3:
            xhci_init_controller(i);
            break;
        }
    }
...
void mouse_set_acceleration(bool enabled, int numerator, int denominator, int threshold)
{
    ...
    if (numerator > 0)
        mouse.accel_numerator = numerator;
    if (denominator > 0)
        mouse.accel_denominator = denominator;
    if (threshold >= 0)
        mouse.accel_threshold = threshold;
}
...
{
    if (total)
        *total = mouse.total_packets;
    if (dropped)
        *dropped = mouse.dropped_packets;
    if (sync_errors)
        *sync_errors = mouse.sync_errors;
}
