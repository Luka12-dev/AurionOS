...
static void notepad_on_draw(Window *win) {
    ...
    for (int i = 0; i < state->text_len; i++) {
        ...
        if (c == '\n') {
            ...
            if (x + 8 > max_x) { x = 8; y += line_h; }
            if (y + line_h > ch) break;
    ...
    if (state->saveas_open) {
        ...
        wm_draw_string(win, dx + 10, dy + 36, "Path (e.g. C:\\myfile.txt):", 0xFF333333, 0xFFF5F5F5);
...
static void paint_on_draw(Window *win) {
    ...
    /* Background */
    wm_fill_rect(win, 0, 0, cw, ch, 0xFF282838);
    ...
    /* Brush size buttons */
    int bs_x = cw - 90;
    wm_draw_string(win, bs_x, 8, "Size:", 0xFFE0E0F0, 0xFF282838);
    for (int s = 1; s <= 3; s++) {
...
typedef struct {
    char path[128];          /* current directory being shown */
    char entries[FILEBROW_MAX_ENTRIES][FILEBROW_NAME_MAX];
    uint8_t entry_type[FILEBROW_MAX_ENTRIES]; /* 0=file,1=dir */
...
static int fb_str_cmp(const char *a, const char *b) {
    while (*a && *b && *a == *b) { a++; b++; }
...
static void fb_scan(FileBrowState *state) {
    ...
    for (int i = 0; i < fs_count && state->entry_count < FILEBROW_MAX_ENTRIES; i++) {
        ...
        /* Must start with our path */
        if (!fb_starts_with(name, state->path)) continue;
        /* The remainder after path must not contain a backslash (direct child only) */
        const char *rest = name + path_len;
        if (rest[0] == 0) continue; /* This is the dir itself */
        /* Check no further subdirectory separators */
        bool sub = false;
...
static void filebrow_on_draw(Window *win) {
    ...
    wm_fill_rect(win, 0, 0, cw, 22, 0xFF2D2D3F);
    wm_draw_string(win, 6, 7, state->path, 0xFFE0E0F0, 0xFF2D2D3F);

    /* Column headers */
    wm_fill_rect(win, 0, 22, cw, 16, 0xFFD8D8E8);
    wm_draw_string(win, 6, 25, "Name", 0xFF000000, 0xFFD8D8E8);
    wm_draw_string(win, cw - 80, 25, "Type", 0xFF000000, 0xFFD8D8E8);
    wm_draw_string(win, cw - 40, 25, "Size", 0xFF000000, 0xFFD8D8E8);
    /* Separator */
    wm_fill_rect(win, 0, 38, cw, 1, 0xFFAAAAAA);

    int row_h = 18;
    int start_y = 40;
    int visible = (ch - start_y) / row_h;

    /* Up directory entry */
    if (fb_str_len(state->path) > 3) { /* not root C:\ */
        wm_fill_rect(win, 0, start_y, cw, row_h, 0xFFF0F0FF);
        wm_draw_string(win, 6, start_y + 4, "[..]", 0xFF0000CC, 0xFFF0F0FF);
        start_y += row_h;
        visible--;
    }
    ...
    for (int i = state->scroll; i < state->entry_count && visible > 0; i++, visible--) {
        ...
        uint32_t bg = sel ? 0xFF4A6CF7 : ((i % 2 == 0) ? 0xFFFFFFFF : 0xFFF5F5FF);
        uint32_t fg = sel ? 0xFFFFFFFF : 0xFF000000;

        wm_fill_rect(win, 0, ey, cw, row_h, bg);

        /* Icon char */
        char icon = (state->entry_type[i] == 1) ? 'D' : 'F';
        wm_draw_char(win, 4, ey + 4, (uint8_t)icon,
                     state->entry_type[i] == 1 ? 0xFF886600 : 0xFF006688, bg);

        wm_draw_string(win, 18, ey + 4, state->entries[i], fg, bg);

        /* Type label */
        const char *type_label = state->entry_type[i] == 1 ? "DIR" : "FILE";
        wm_draw_string(win, cw - 80, ey + 4, type_label, fg, bg);
        ...
        if (state->entry_type[i] == 0) {
            ...
            if (s == 0) {
                ...
                int ti=0; uint32_t tmp=s;
                char tmp2[12];
                while(tmp>0){tmp2[ti++]='0'+(tmp%10);tmp/=10;}
                for(int k=0;k<ti;k++) sz[k]=tmp2[ti-1-k];
                sz[ti]=0;
            }
            wm_draw_string(win, cw - 40, ey + 4, sz, fg, bg);
        }
    }

    /* Empty message */
    if (state->entry_count == 0) {
        wm_draw_string(win, cw/2 - 40, ch/2, "(empty)", 0xFF808090, 0xFFFFFFFF);
    }
    ...
    char nbuf[8];
    if (ec == 0) { nbuf[0]='0'; nbuf[1]=0; }
    else { int ti=0; int tmp=ec; char tmp2[8];
        while(tmp>0){tmp2[ti++]='0'+(tmp%10);tmp/=10;}
        for(int k=0;k<ti;k++) nbuf[k]=tmp2[ti-1-k]; nbuf[ti]=0; }
...
static void filebrow_on_mouse(Window *win, int lx, int ly, bool left, bool right) {
    ...
    /* Track double-click: two clicks within ~400ms on same item */
    ...
    if (has_up) start_y += row_h; /* account for [..] row */

    /* Check [..] up row */
    if (has_up && ly >= 40 && ly < 40 + row_h) {
        int pl = fb_str_len(state->path);
        if (pl > 3) {
            int i = pl - 2;
            while (i > 2 && state->path[i] != '\\') i--;
            state->path[i + 1] = 0;
            fb_scan(state);
            fb_last_click_idx = -1;
        }
    ...
    bool is_double = (clicked_idx == fb_last_click_idx &&
                      (now - fb_last_click_tick) < 40); /* ~40 ticks @ 100Hz = 400ms */
...
static void filebrow_on_key(Window *win, uint16_t key) {
    ...
    if ((key & 0xFF) == 8) {
        int pl = fb_str_len(state->path);
        if (pl > 3) {
            /* Remove trailing backslash then find previous one */
            int i = pl - 2;
            while (i > 2 && state->path[i] != '\\') i--;
            state->path[i+1] = 0;
            fb_scan(state);
...
void app_filebrowser_create(void) {
    ...
    Window *win = wm_create_window("Files", x, y, w, h);
    if (!win) return;
...
static void clock_on_draw(Window *win) {
    ...
    wm_fill_rect(win, 0, 0, cw, ch, 0xFF1A1A2E);
    ...
    wm_draw_string(win, tx, ty, time_str, 0xFF4ADE80, 0xFF1A1A2E);
    ...
    wm_draw_string(win, (cw - pos * 8) / 2, ty + 30, date_str, 0xFF808090, 0xFF1A1A2E);
}
...
static void sysinfo_on_draw(Window *win) {
    ...
    int y = 10;
    wm_draw_string(win, 10, y, "Aurion OS v1.0 Beta", 0xFF4ADE80, 0xFF1A1A2E);
    y += 20;

    wm_draw_string(win, 10, y, "32-bit x86 Operating System", 0xFFE0E0F0, 0xFF1A1A2E);
    y += 20;
    ...
    wm_draw_string(win, 10, y, buf, 0xFF4ADE80, 0xFF1A1A2E);
    y += 18;

    /* Total Used */
    str_copy(buf, "Used: ", 64);
    len = str_len(buf);
    ...
    len = str_len(buf);
    str_copy(buf + len, " MB)", 64 - len);
    wm_draw_string(win, 10, y, buf, 0xFFE0E0F0, 0xFF1A1A2E);
    y += 24;
...
typedef struct {
    int tab; /* 0=Appearance, 1=System, 2=Dock */
    int scroll;
} SettingsState;
...
static void settings_on_draw(Window *win) {
    ...
    /* Sidebar */
    int sb_w = 120;
    wm_fill_rect(win, 0, 0, sb_w, ch, 0xFF1A1A2E);
    wm_fill_rect(win, sb_w - 1, 0, 1, ch, 0xFF303050);

    const char *tabs[] = {"Appearance", "System", "Dock"};
    for (int i = 0; i < 3; i++) {
        ...
        wm_draw_string(win, 10, 20 + i * 36, tabs[i], sel ? 0xFFFFFFFF : 0xFF808090, 
                       sel ? WM_COLOR_ACCENT : 0xFF1A1A2E);
    }
    ...
    if (state->tab == 0) {
        /* Appearance */
        wm_draw_string(win, content_x, y, "Background Color:", 0xFFFFFFFF, WM_COLOR_WINDOW_BG);
        y += 24;
        const char *colors[] = {"Blue", "Green", "Red", "White", "Black"};
        for (int i = 0; i < 5; i++) {
            bool sel = (g_settings.bg_color == (uint32_t)i);
            wm_fill_rect(win, content_x, y, 100, 24, sel ? WM_COLOR_ACCENT : 0xFF2D2D3F);
            wm_draw_string(win, content_x + 10, y + 6, colors[i], 0xFFFFFFFF, 
                           sel ? WM_COLOR_ACCENT : 0xFF2D2D3F);
            y += 28;
        }
        ...
        for (int i = 0; i < 2; i++) {
            ...
            wm_fill_rect(win, content_x, y, 140, 24, sel ? WM_COLOR_ACCENT : 0xFF2D2D3F);
        ...
        y += 24;
        const char *apps[] = {"None", "Terminal", "Notepad", "Paint", "Calculator", "Files", "Clock", "SysInfo"};
        for (int i = 0; i < 8; i++) {
            bool sel = (g_settings.startup_app_idx == i - 1);
            wm_fill_rect(win, content_x, y, 120, 24, sel ? WM_COLOR_ACCENT : 0xFF2D2D3F);
            wm_draw_string(win, content_x + 10, y + 6, apps[i], 0xFFFFFFFF, 
                           sel ? WM_COLOR_ACCENT : 0xFF2D2D3F);
            y += 28;
            if (y > ch - 40) { y = 44; content_x += 140; }
        }
    } else if (state->tab == 2) {
        /* Dock */
...
static void settings_on_mouse(Window *win, int lx, int ly, bool left, bool right) {
    ...
    if (state->tab == 0) {
        ...
        y += 24;
        for (int i = 0; i < 8; i++) {
            if (lx >= content_x && lx < content_x + 120 && ly >= y && ly < y + 24) {
                g_settings.startup_app_idx = i - 1;
                wm_save_settings();
                return;
            }
            y += 28;
            if (y > wm_client_h(win) - 40) { y = 44; content_x += 140; }
        }
    } else if (state->tab == 2) {
        /* Dock - Magnification */
        y += 24;
...
static void snake_on_key(Window *win, uint16_t key) {
    ...
    if (state->game_over) {
        if (key == 'r' || key == 'R' || key == 0x1C) { /* R or Enter */
            snake_reset(state);
        }
        return;
    }
...
static void snake_on_draw(Window *win) {
    ...
    /* Game logic update */
    if (!state->game_over && (now - state->last_move_ticks > 8)) {
    ...
    if (state->game_over) {
        wm_fill_rect(win, cw / 4, ch / 2 - 30, cw / 2, 60, 0xCC1A1A2E);
        wm_draw_string(win, cw / 2 - 40, ch / 2 - 15, "GAME OVER", 0xFFEF4444, 0x00000000);
        wm_draw_string(win, cw / 2 - 60, ch / 2 + 5, "Press R to Restart", 0xFFFFFFFF, 0x00000000);
    }
}
