...
extern PyObject *py_new_int(int64_t val);
extern PyObject *py_new_float(double val);
extern PyObject *py_new_bool(bool val);
extern PyObject *py_new_str(const char *s);
extern PyObject *py_new_strn(const char *s, size_t len);
extern PyObject *py_new_list(size_t capacity);
extern PyObject *py_new_dict(void);
extern PyObject *py_new_tuple(size_t size);
extern PyObject *py_new_set(void);
extern PyObject *py_new_module(PyObject *name);
extern PyObject *py_call(PyObject *func, PyObject **args, size_t nargs);
extern int py_setitem(PyObject *o, PyObject *key, PyObject *val);
extern const char *py_str_cstr(PyObject *o);
extern int64_t py_to_int(PyObject *o);
extern double py_to_float(PyObject *o);
extern bool py_is_true(PyObject *o);
extern PyObject *py_getattr(PyObject *o, const char *name);
extern int py_setattr(PyObject *o, const char *name, PyObject *val);
extern PyObject *py_len(PyObject *o);
extern PyObject *py_get_builtins(void);
extern int PyDict_SetItemString(PyObject *o, const char *key, PyObject *val);
extern PyObject *PyDict_GetItemString(PyObject *o, const char *key);
extern int PyList_Append(PyObject *o, PyObject *v);
...
static PyObject *pyos_urandom(PyObject *self, PyObject **args, size_t nargs) {
    (void)self;
    size_t n = 16;
    if (nargs >= 1) n = (size_t)py_to_int(args[0]);
    if (n > 256) n = 256;
    uint8_t *buf = (uint8_t *)malloc(n);
    for (size_t i = 0; i < n; i++) buf[i] = (uint8_t)((get_ticks() + i * 17) & 0xFF);
...
static PyObject *pystr_repr(PyObject *self, PyObject **args, size_t nargs) {
    ...
    if (obj->type == PY_NONE) return py_new_str("None");
    if (obj->type == PY_BOOL) return py_new_str(obj->val.bool_val ? "True" : "False");
...
static PyObject *pystr_upper(PyObject *self, PyObject **args, size_t nargs) {
    (void)self;
    if (nargs < 1) return py_new_str("");
    if (args[0]->type != PY_STR) return py_new_str("");
...
static PyObject *pystr_find(PyObject *self, PyObject **args, size_t nargs) {
    (void)self;
    if (nargs < 2) return py_new_int(-1);
    return py_new_int(-1);
}
...
static PyObject *pymath_log(PyObject *self, PyObject **args, size_t nargs) {
    (void)self;
    if (nargs < 1) return py_new_float(0.0);
    if (nargs >= 2) return py_new_float(log(py_to_float(args[0])) / log(py_to_float(args[1])));
...
static PyObject *pymath_isnan(PyObject *self, PyObject **args, size_t nargs) {
    (void)self;
    if (nargs < 1) return py_new_bool(false);
    double v = py_to_float(args[0]);
...
static PyObject *pymath_lcm(PyObject *self, PyObject **args, size_t nargs) {
    ...
    int64_t a = py_to_int(args[0]);
    int64_t b = py_to_int(args[1]);
    if (a == 0 || b == 0) return py_new_int(0);
...
void py_init_stdlib(void) {
    ...
    Py_INCREF(os_mod); Py_INCREF(sys_mod); Py_INCREF(time_mod);
    Py_INCREF(math_mod); Py_INCREF(random_mod);
}
